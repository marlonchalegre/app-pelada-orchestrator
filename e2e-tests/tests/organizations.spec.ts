...
test.describe("Organization Management", () => {
  ...
  test("should manage organization, invitations, and admins", async ({
    browser,
  }, testInfo) => {
    ...
    await test.step("2. Personal Invitation & First Access Flow", async () => {
      const invitationLinkText = await invitePlayerByEmail(
        ownerPage,
        invitedUser.email,
      );
      ...
      await invitedPage
        .getByTestId("first-access-username")
        .fill(invitedUser.username);
      await invitedPage
        .getByTestId("first-access-password")
        .fill(invitedUser.password);
      await invitedPage.getByTestId("first-access-position-select").click();
      await invitedPage
        .getByTestId(`position-option-${invitedUser.position}`)
        .click();
    ...
    await test.step("3. Admin Management & Revoke", async () => {
      ...
      await expect(ownerPage.getByTestId("admin-select")).toBeVisible({
        timeout: 15000,
      });
      ...
      await expect(
        ownerPage.locator(`text=${invitedUser.name}`).last(),
      ).toBeVisible();
      ...
      await ownerPage
        .getByRole("option", { name: "Mensalista", exact: true })
        .click();
      ...
      await ownerPage
        .getByTestId("invite-dialog-close-button")
        .click({ force: true });
    ...
    await test.step("4. Public Link & Leave Flow", async () => {
      ...
      await expect(
        ownerPage.getByTestId("public-invite-link-text"),
      ).toBeVisible({ timeout: 15000 });
      const publicLink = (
        await ownerPage.getByTestId("public-invite-link-text").innerText()
      ).trim();
      ...
      await expect(joinerPage.getByTestId("join-org-button")).toBeVisible({
        timeout: 15000,
      });
      ...
      await expect(
        joinerPage.getByTestId(`org-link-${orgName}`),
      ).not.toBeVisible();
      ...
      await ownerPage
        ...
        .click({ force: true });
      await expect(
        ownerPage.getByRole("dialog", { name: /invite/i }),
    ...
    await test.step("5. Public Link Reset and List Filtering", async () => {
      ...
      const publicLinkText = await ownerPage
        .getByTestId("public-invite-link-text")
        .innerText();
      expect(publicLinkText).toContain("/join/");
      await expect(
        ownerPage
          .locator("li")
          .filter({ hasText: /convite público|public invite/i }),
      ).toBeHidden();

      await ownerPage.getByTestId("reset-public-link-button").click();
      await expect(ownerPage.getByRole("dialog")).toBeVisible();
      await expect(
        ownerPage.getByText(/tem certeza|are you sure/i),
      ).toBeVisible();
      ...
      await expect(
        ownerPage.getByTestId("public-invite-link-text"),
      ).not.toHaveText(publicLinkText, { timeout: 10000 });

      const newPublicLinkText = await ownerPage
        .getByTestId("public-invite-link-text")
        .innerText();
    ...
    await test.step("6. Player Ratings Management", async () => {
      ...
      const ratingInput = playerRow
        .getByTestId(/^rating-input-/)
        .locator("input");
    ...
    await test.step("7. Delete Organization", async () => {
      ...
      await expect(
        ownerPage.getByTestId(`org-link-${orgName}`),
      ).not.toBeVisible();
    });
  ...
  test("should verify attendance details, restricted icons and row visibility", async ({
    browser,
  }) => {
    ...
    await playerPage
      .getByRole("tab", { name: /Espera|Waitlist/i })
      .first()
      .click();
    await expect(
      playerPage.getByTestId(`attendance-card-${playerUser.username}`),
    ).toBeVisible();
    await expect(
      playerPage.getByTestId("attendance-card-confirm"),
    ).not.toBeVisible();
    await expect(
      playerPage.getByTestId("attendance-card-decline"),
    ).not.toBeVisible();
    ...
    await expect(
      playerPage.getByTestId("copy-players-button"),
    ).not.toBeVisible();
    await expect(
      playerPage.getByTestId("randomize-teams-button"),
    ).not.toBeVisible();
    await expect(
      playerPage.getByTestId("create-team-button"),
    ).not.toBeVisible();
    await expect(
      playerPage.getByTestId("share-dropdown-button"),
    ).not.toBeVisible();
  ...
  test("should verify mobile optimizations (hidden text)", async ({ page }) => {
    ...
    const textSpan = closeBtn
      .locator("span")
      .filter({ hasText: /Fechar Lista|Close List/i });
    ...
    const tabLabel = page
      ...
      .locator("span")
      .filter({ hasText: /Membros|Members/i });
  ...
  test("should configure WAHA settings", async ({ page }, testInfo) => {
    ...
    await page
      .getByLabel(
        /(Notify when pelada starts|Notificar quando a pelada iniciar)/i,
      )
      .click();
    await page
      .getByLabel(
        /(Notify when pelada ends|Notificar quando a pelada encerrar)/i,
      )
      .click();
    ...
    await expect(
      page.getByText(
        /(saved successfully|salva com sucesso|salvas com sucesso)/i,
      ),
    ).toBeVisible();
    ...
    await page.getByTestId("mgmt-tab-waha").click();
    await expect(enabledSwitch).toBeChecked();
    await expect(page.locator('input[name="waha_api_url"]')).toHaveValue(
      "http://waha:3000",
    );
    await expect(page.locator('input[name="waha_instance"]')).toHaveValue(
      "default",
    );
    await expect(page.locator('input[name="waha_group_id"]')).toHaveValue(
      "123456789@g.us",
    );
    await expect(
      page.locator('input[name="waha_start_msg_enabled"]'),
    ).toBeChecked();
    await expect(
      page.locator('input[name="waha_end_msg_enabled"]'),
    ).toBeChecked();

    await saveVideo(page, "waha-configuration", testInfo);
  });

  test("should redirect existing member to org page when accessing invite link", async ({
    browser,
  }) => {
  ...
  test("should configure priority confirmation limit hours and default max players in organization settings", async ({
    browser,
  }) => {
    const timestamp = Date.now() + Math.floor(Math.random() * 1000000);
    const owner = {
    ...
    const createPeladaMaxInput = page
