...
test.describe("Premium Feature Flags Workflows", () => {
  ...
  test("Should restrict premium features initially and unlock them when toggled by Global Admin", async ({
    browser,
  }, testInfo) => {
    ...
    await test.step("2. Verify premium features are locked initially", async () => {
      ...
      await expect(
        orgAdminPage.getByText(/Avaliações e Características de Jogadores/i),
      ).toBeVisible();

      // Click tab: WAHA
      await orgAdminPage.getByTestId("mgmt-tab-waha").click();
      await expect(
        orgAdminPage.getByText(/Comunicações Automatizadas via WhatsApp/i),
      ).toBeVisible();
    });
    ...
    await test.step("3. Login as Global Admin and enable feature flags", async () => {
      ...
      await orgRow
        .locator(`[data-testid="manage-feature-flags-btn-${orgId}"]`)
        .click();
      ...
      await globalAdminPage
        .getByTestId("switch-player_characteristics")
        .click();
      ...
      const savePromise = globalAdminPage.waitForResponse(
        (resp) =>
          resp
            .url()
            .includes(`/api/admin/organizations/${orgId}/feature-flags`) &&
          resp.status() === 200,
      );
      await globalAdminPage
