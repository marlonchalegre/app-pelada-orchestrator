...
test.describe("Global Admin Panel & Block Systems", () => {
  ...
  test("Global Admin and Blocking Workflows", async ({ browser }, testInfo) => {
    ...
    await test.step("2. Promote user to Global Admin and access panel", async () => {
      ...
      await globalAdminPage
        .getByTestId("login-email")
        .fill(globalAdminUser.username);
      await globalAdminPage
        .getByTestId("login-password")
        .fill(globalAdminUser.password);
      ...
      await expect(globalAdminPage.locator("h1")).toContainText(
        /Painel do Global Admin|Global Admin Panel/i,
      );
    });
    ...
    await test.step("2.5. Edit user email and phone", async () => {
      ...
      const userSearchInput = globalAdminPage.getByPlaceholder(
        /Buscar por nome|Search by name/i,
      );
      await userSearchInput.click();
      await userSearchInput.clear();
      await userSearchInput.pressSequentially(regularUser.name);
      const userResponsePromise = globalAdminPage.waitForResponse(
        (resp) =>
      ...
      const userRow = globalAdminPage
        .locator("tr")
        .filter({ hasText: regularUser.name });
      ...
      const savePromise = globalAdminPage.waitForResponse(
        (resp) => resp.url().includes("/profile") && resp.status() === 200,
      );
    ...
    await test.step("3. Toggle organization creation permission", async () => {
      ...
      const orgCreationSwitch = userRow
      ...
      const grantTogglePromise = globalAdminPage.waitForResponse(
        (resp) =>
          resp.url().includes("/toggle-org-creation") && resp.status() === 200,
      );
      ...
      const revokeTogglePromise = globalAdminPage.waitForResponse(
        (resp) =>
          resp.url().includes("/toggle-org-creation") && resp.status() === 200,
      );
      await orgCreationSwitch.click();
    ...
    await test.step("4. Block user and verify restrictions", async () => {
      ...
      // Search for regular user
      const userSearchInput2 = globalAdminPage.getByPlaceholder(
        /Buscar por nome|Search by name/i,
      );
      await userSearchInput2.click();
      await userSearchInput2.pressSequentially(regularUser.name);
      const userResponsePromise2 = globalAdminPage.waitForResponse(
        (resp) =>
          resp.url().includes("/api/users/search") && resp.status() === 200,
      );
      ...
      await expect(
        regularPage.getByText(
          /Sua conta está bloqueada|Your account is blocked/i,
        ),
      ).toBeVisible();
      ...
      await expect(
        regularPage.getByTestId("create-org-open-dialog"),
      ).toBeHidden();
    ...
    await test.step("5. Block organization and check restrictions", async () => {
      ...
      await globalAdminPage.goto("/admin");
      await globalAdminPage
        .getByRole("tab", { name: /Organizações|Organizations/i })
        .click();

      // Search for the organization
      const orgSearchInput = globalAdminPage.getByPlaceholder(
        /Buscar organização|Search organization/i,
      );
      ...
      const orgResponsePromise = globalAdminPage.waitForResponse(
        (resp) =>
          resp.url().includes("/api/admin/organizations") &&
          resp.status() === 200,
      );
      ...
      await expect(
        orgAdminPage.getByTestId("org-blocked-banner"),
      ).toBeVisible();

      // The create pelada form/section should be hidden
      await expect(
        orgAdminPage.getByTestId("create-pelada-submit"),
      ).toBeHidden();
    });
    ...
    await test.step("6. Reset user password and verify login", async () => {
      ...
      // Click confirm
      const resetPromise = globalAdminPage.waitForResponse(
        (resp) =>
          resp.url().includes("/reset-password") && resp.status() === 200,
      );
      ...
      await regularPage
        .getByTestId("login-email")
        .fill(updatedRegularUser.username);
      await regularPage
        .getByTestId("login-password")
        .fill(updatedRegularUser.password);
    ...
    await test.step("7. Add and remove organization admins", async () => {
      ...
      await globalAdminPage
        .getByTestId("admin-search-input")
        .fill(regularUser.username);
      const searchUsersPromise = globalAdminPage.waitForResponse(
        (resp) =>
          resp.url().includes("/api/users/search") && resp.status() === 200,
      );
      ...
      const addAdminPromise = globalAdminPage.waitForResponse(
        (resp) => resp.url().includes("/admins") && resp.status() === 200,
      );
      ...
      await expect(regularPage.getByTestId("admin-orgs-list")).toContainText(
        orgName,
      );

      // Now remove the regularUser from admins on globalAdminPage
      const removeAdminPromise = globalAdminPage.waitForResponse(
        (resp) => resp.url().includes("/admins/") && resp.status() === 200,
      );
      ...
      await dialog
        .locator("li")
        .filter({ hasText: regularUser.name })
        .locator('[data-testid^="remove-admin-btn-"]')
        .click();
      ...
      const lastAdminItem = dialog
        .locator("li")
        .filter({ hasText: orgAdminUser.name });
      const removeLastAdminBtn = lastAdminItem.locator(
    ...
    await test.step("8. Remove user and check cascading deletion", async () => {
      await globalAdminPage.goto("/admin");
      await globalAdminPage
        .getByRole("tab", { name: /Usuários|Users/i })
        .click();
      ...
      const deletePromise = globalAdminPage.waitForResponse(
        (resp) => resp.url().includes("/api/user/") && resp.status() === 204,
      );
      ...
      await expect(
        globalAdminPage.locator("tr").filter({ hasText: regularUser.name }),
      ).toBeHidden();
    ...
    await test.step("9. Remove organization and check deletion", async () => {
      ...
      const deletePromise = globalAdminPage.waitForResponse(
        (resp) =>
          resp.url().includes("/api/organizations/") && resp.status() === 200,
      );
      ...
      await expect(deleteDialog).toBeHidden();
      await expect(
        globalAdminPage.locator("tr").filter({ hasText: orgName }),
      ).toBeHidden();
      ...
      await orgAdminPage.waitForLoadState("networkidle");
      await expect(
        orgAdminPage.getByTestId("admin-orgs-list"),
