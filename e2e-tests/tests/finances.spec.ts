...
import { registerAndCreateOrg, createPelada, makeMensalista } from "./utils";
...
async function navigateToOrgManagement(page: Page, orgName: string) {
  ...
  await expect(
    page
      .getByTestId("org-mgmt-loading")
      .or(page.getByTestId("org-mgmt-container")),
  ).toBeVisible();
...
async function goToFinanceTab(page: Page) {
  ...
  await expect(page.getByTestId("finance-loading")).not.toBeVisible({
    timeout: 20000,
  });
}

test.describe("Financial Control & Fines", () => {
  test("should manage organization finances, transactions and payments", async ({
    browser,
  }) => {
    ...
    await test.step("2. Manual Transactions", async () => {
      ...
      await expect(page.getByTestId("summary-balance-value")).toHaveAttribute(
        "data-amount",
        "-50",
      );
    ...
    await test.step("3. Diarista Payout Flow", async () => {
      ...
      let allPlayerNames = await page
        .locator('[data-testid^="attendance-card-"]')
        .all();
      ...
      const dashboardDataPromise = page.waitForResponse(
        (response) => {
          ...
          return (
            (url.includes("dashboard-data") || url.includes("full-details")) &&
            response.status() === 200
          );
        },
        { timeout: 30000 },
      ...
      await expect(
        page.getByText(/Pagamento Pelada \d{2}\/\d{2}\/\d{4}/),
      ).toBeVisible();
      await expect(page.getByTestId("summary-balance-value")).toHaveAttribute(
        "data-amount",
        "175",
    ...
    await test.step("4. Mensalista Monthly Fee Flow", async () => {
      ...
      const playerRow = page
        .getByTestId(/monthly-payment-row-.*/)
      ...
      await expect(
        page.getByText(`Mensalidade ${currentMonth}/${currentYear}`, {
          exact: true,
        }),
      ).toBeVisible();
      await expect(page.getByTestId("summary-balance-value")).toHaveAttribute(
  ...
  test.describe("Automatic Fine", () => {
    ...
    test("should apply fine automatically after day 5", async ({ page }) => {
      ...
      const playerRow = page
        .getByTestId(/monthly-payment-row-.*/)
        .filter({ hasText: fineOwner.name });
      await expect(playerRow.getByText(/[\$R]\s*115[,\.]00/)).toBeVisible({
        timeout: 20000,
      });
      ...
      await expect(
        playerRow.locator("text=+ R$ 15,00 (multa)"),
      ).not.toBeVisible();
      ...
      await page.getByTestId("finance-tab-transactions").click();
      await expect(
        page.getByText("Mensalidade 5/2026", { exact: true }),
      ).toBeVisible({ timeout: 10000 });
      await expect(
        page.getByText("Multa Mensalidade 5/2026", { exact: true }),
      ).toBeVisible();

      const txRow = page
        .locator("tr")
        .filter({ hasText: "Mensalidade 5/2026" })
        .filter({ hasNotText: "Multa" });
      await expect(txRow.getByText(/100[,\.]00/)).toBeVisible();

      const fineRow = page
        .locator("tr")
        .filter({ hasText: "Multa Mensalidade 5/2026" });
  ...
  test.describe("Manual Fine Control", () => {
    ...
    test("should allow admin to choose not to apply fine after deadline", async ({
      page,
    }) => {
      ...
      const playerRow = page
        .getByTestId(/monthly-payment-row-.*/)
        .filter({ hasText: manualOwner.name });
      ...
      await expect(
        page.getByTestId("apply-fine-checkbox").locator("input"),
      ).toBeChecked();
      ...
      await expect(
        playerRow.locator("text=+ R$ 20,00 (multa)"),
      ).not.toBeVisible();
      ...
      await expect(txRow.getByText(/100[,\.]00/)).toBeVisible();
      await expect(
        page.locator("tr").filter({ hasText: "Multa Mensalidade" }),
      ).not.toBeVisible();
    });

    test("should still apply fine if admin leaves checkbox checked", async ({
      page,
    }) => {
      ...
      await expect(
        playerRow.getByText(/\+.*20[,\.\$]00.*(multa|fine)/i),
      ).toBeVisible();
      ...
      await expect(
        page
          .locator("tr")
          .filter({ hasText: /Mensalidade|Monthly Fee/ })
          .getByText(/100[,\.]00/),
      ).toBeVisible();
      await expect(
        page
          .locator("tr")
          .filter({ hasText: /Multa Mensalidade|Monthly Fee Fine/ })
          .getByText(/20[,\.]00/),
      ).toBeVisible();
    });
    ...
    test("should allow reversing the fine independently", async ({ page }) => {
      ...
      await expect(
        page
          .locator("tr")
          .filter({ hasText: "Mensalidade" })
          .getByText(/100[,\.]00/),
      ).toBeVisible();
      const fineRow = page
        .locator("tr")
        .filter({ hasText: "Multa Mensalidade" });
      await expect(fineRow.getByText(/20[,\.]00/)).toBeVisible();

      await fineRow
        .locator('[data-testclass="reverse-transaction-button"]')
        .click();
      await page.getByTestId("pretty-confirm-button").click();

      await expect(page.getByTestId("finance-success")).toBeVisible();
      await expect(page.getByTestId("finance-success")).toContainText(
        /(sucesso|successfully)/i,
      );

      await expect(fineRow.getByTestId("transaction-amount")).toHaveCSS(
