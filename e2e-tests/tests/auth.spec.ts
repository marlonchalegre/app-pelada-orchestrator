...
test.describe("Auth & Profile", () => {
  ...
  test("should register, update profile (including phone), and delete account", async ({
    browser,
  }, testInfo) => {
    ...
    await test.step("Update Profile", async () => {
      ...
      const switchLabel = page.getByText(
        /Notificações para Diaristas e Convidados|Notifications for Casual Players/i,
      );
      const nonMensalistaSwitch = page.getByTestId(
      ...
      await expect(
        page.getByText(
          /Profile updated successfully|Perfil atualizado com sucesso/i,
        ),
      ).toBeVisible();
      ...
      await expect(
        page.getByTestId("receive-non-mensalista-updates-switch"),
      ).toBeChecked();
    });
  ...
  test("should register without phone and then add it in profile", async ({
    browser,
  }, testInfo) => {
  ...
  test("should upload, view, and delete profile picture", async ({
    browser,
  }, testInfo) => {
    ...
    await test.step("Upload Avatar", async () => {
      ...
      await expect(page.getByTestId("profile-success-alert")).toContainText(
        /Profile picture updated|Foto de perfil atualizada/i,
      );
  ...
  test("should complete the password reset flow", async ({
    page,
  }, testInfo) => {
    ...
    await page
      .getByRole("link", { name: /Forgot your password|Esqueceu sua senha/i })
      .click();
    ...
    await expect(
      page.getByText(
        /If an account with (that|this) email exists|Se uma conta com este email existir/i,
      ),
    ...
    await expect(
      page.getByText(/Passwords do not match|As senhas não coincidem/i),
    ).toBeVisible();

    await saveVideo(page, "password-reset-flow", testInfo);
  });

  test("should toggle password visibility in Login, Register, and Reset Password pages", async ({
    page,
  }) => {
  ...
  test.describe("Auth Redirection Bugs", () => {
    ...
    test("unauthenticated user should be redirected from protected page to welcome page", async ({
      page,
    }) => {
      await page.goto("/login");
    ...
    test("authenticated user should be redirected from login page to home", async ({
      browser,
    }) => {
    ...
    test("authenticated user should be redirected from register page to home", async ({
      browser,
    }) => {
      const context = await browser.newContext();
      const page = await context.newPage();
    ...
    test("user with invalid token should be redirected to welcome page", async ({
      browser,
    }) => {
      const context = await browser.newContext();
      const page = await context.newPage();
      ...
      await page.evaluate(() => {
        localStorage.setItem(
          "authToken",
          "invalid-token-header.invalid-payload.invalid-signature",
        );
        localStorage.setItem(
          "authUser",
          JSON.stringify({ id: 1, name: "Fake User", username: "fake" }),
        );
      });
